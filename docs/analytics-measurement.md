...
## GA4-only production launch

The verified GA4 stream can go live with the Meta Pixel ID set to null. In this state the Meta script cannot load, the marketing choice is hidden, and even an earlier saved choice cannot grant marketing consent. The five-language privacy notice and contact details (`Gil Sher`, `gil.sher@gmail.com`) should remain accurate for the site operator.

To activate Meta later, complete the business-portfolio/ad-account association, create and verify the correct Dataset/Pixel, enter its numeric ID into `analytics-config.js`, and test Meta Test Events after explicit marketing consent. The changed consent key will prompt visitors to choose again.
...
1. In a fresh browser session, visit a Facebook-tagged URL such as `https://wroc-love.com/?lang=he&utm_source=facebook&utm_medium=paid_social&utm_campaign=wroclaw-water-tower-maps&utm_content=water-tower-he-facebook`. Decline consent and confirm there are no GA4 or Meta tag requests. Grant analytics and confirm one `page_view` in GA4 DebugView with Hebrew and the campaign values. After Meta activation, grant marketing and confirm one `PageView` in Meta Test Events.
