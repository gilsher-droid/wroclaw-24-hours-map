...
const sources = [
  { productId: "wroclaw-24-hours", rank: 1, records: [...main.LOCATIONS, ...main.EVENING_LOCATIONS], kind: "route-stop" },
  { productId: "lifestyle-guide", rank: 2, records: lifestyle.WROC_LIFESTYLE_PLACES, kind: "catalog-place" },
  { productId: "wroclaw-four-days", rank: 3, records: premium.PREMIUM_STOPS, kind: "route-stop" },
  { productId: "wroclaw-four-days", rank: 3, records: premium.PREMIUM_RECOMMENDATIONS, kind: "recommendation" },
  { productId: "wroclaw-christmas", rank: 4, records: christmas.PREMIUM_STOPS, kind: "route-stop" },
  { productId: "wroclaw-christmas", rank: 4, records: christmas.PREMIUM_RECOMMENDATIONS, kind: "recommendation" },
];
...
for (const source of sources) {
  for (const record of source.records) {
    ...
    const canonicalId = aliases[record.id] || record.id;
...
for (const place of places.values()) {
  ...
  place.taxonomy = { categories: place.categories, tags: [] };
  place.suitability = {
    recommendedFor: [],
  ...
  place.media = {
    photos: Array.isArray(resources.gallery) ? resources.gallery : [],
  ...
  place.languages = ["he", "en", "pl", "de", "cs"];
  delete place._rank;
}
...
await writeFile(resolve(root, "data/place-catalog.js"), banner + runtime);
